...
# USERNAME
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/rasta-team/allow/main/ipvps.conf | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
# Order ID
rm -f /usr/bin/ver
user=$(curl https://raw.githubusercontent.com/rasta-team/allow/main/ipvps.conf | grep $MYIP | awk '{print $3}')
echo "$user" >/usr/bin/ver
# validity
rm -f /usr/bin/e
valid=$(curl https://raw.githubusercontent.com/rasta-team/allow/main/ipvps.conf | grep $MYIP | awk '{print $4}')
...
echo -e " \e[$line╘════════════════════════════════════════════════════════════╛\e[m"
echo -e "  \e[$number (•1)\e[m \e[$below XRAY VMESS & VLESS\e[m  \e[$number (•7)\e[m \e[$below REBOOT VPS\e[m"
echo -e "  \e[$number (•2)\e[m \e[$below TROJAN XRAY & WS\e[m    \e[$number (•8)\e[m \e[$below MENU THEMES\e[m"
echo -e "  \e[$number (•3)\e[m \e[$below SSHWS & OPENVPN\e[m     \e[$number (•9)\e[m \e[$below INFO ALL PORT\e[m"
echo -e "  \e[$number (•4)\e[m \e[$below SYSTEM MENU\e[m         \e[$number (10)\e[m \e[$below CLEAR EXPIRED FILES\e[m"
echo -e "  \e[$number (•5)\e[m \e[$below CHECK RUNNING\e[m       \e[$number (11)\e[m \e[$below CLEAR LOG VPS\e[m"
echo -e "  \e[$number (•6)\e[m \e[$below CHANGE PORT\e[m         \e[$number (12)\e[m \e[$below CLEAR SAMPAH\e[m"
echo -e ""
echo -e " \e[$line╒════════════════════════════════════════════════════════════╕\e[m"
echo -e "  \e[$below Version Name         : MULTIPORT/WEBSOCKET v2.0"
echo -e "  \e[$below Autoscript By        : KhaiVPN"
echo -e "  \e[$below Helper admin         : Reyza"
...
11)
    ...
    ;;
12)
    wget https://raw.githubusercontent.com/rasta-team/MultiportV2/main/clearcache.sh && chmod +x clearcache.sh && ./clearcache.sh
    ;;
x)
    clear
    exit
