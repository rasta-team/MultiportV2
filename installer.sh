...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/rasta-team/access/main/ip > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/rasta-team/access/main/ip | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/rasta-team/access/main/ip | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
surat=$(curl -sS https://raw.githubusercontent.com/rasta-team/access/main/bot | grep -w 139.162.32.40 | awk '{print $4}'  )
if [[ "$surat" = "true" ]]; then
echo -ne
else
